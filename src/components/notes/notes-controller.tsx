...
import {updateNote as saveNoteInDB, createNote as createNoteInDB, fetchNotes} from "@/server/db/notes-queries";
...
export default function NotesController() {
    ...
    const saveNote = async () => {
        if(currentNote && notes) {
            await saveNoteInDB(currentNote.id, currentNote.note_title, currentNote.note_content);
    ...
    return (
        <>
            <NotesSidebar notes={notes} setNotes={setNotes} currentNote={currentNote} setCurrentNote={setCurrentNote} createNote={createNote} setMessages={setMessages} />
            <main className="mt-10 w-9/12 mx-auto">
                <div className="flex justify-between">
                    <SidebarTrigger/>
                    <Button className="mr-4 mb-1" onClick={() => setIsAIChatOpen(!isAIChatOpen)} disabled={!currentNote}>
                        {
                            isAIChatOpen ?
                                <>
                                    Return to Note<ArrowLeft />
                                </>
                                :
                                <>
                                    Chat to AI<BotMessageSquare />
                                </>
                        }
                    </Button>
                </div>
                {!isAIChatOpen ?
                    <NotesEditor currentNote={currentNote} setCurrentNote={setCurrentNote} createNote={createNote}
                                 saveNote={saveNote} messages={messages} setMessages={setMessages} />
                    : <div className="h-[75vh] w-full mx-auto mt-2">
                        <Chat messages={messages} input={input} handleInputChange={handleInputChange} handleSubmit={handleSubmit} status={status} />
                    </div>
                }
            </main>
        </>
