...
export function NotesSidebar({ notes, setNotes, currentNote, setCurrentNote, createNote, setMessages }: NotesSidebarProps) {
    return (
        <Sidebar>
            ...
            <SidebarContent className="px-4">
                <Button className="w-9/12" onClick={async () => {
                    await createNote("Untitled Note", "");
                    setMessages([{
                        id: "",
                        role: "system",
                        content: "Hey AI, there is no note content available. Let the user know they need to write something first."
                    }]);
                    ...
                    return (
                        ...
                        >
                            <div
                                className={`flex-1`}
                            >
                                <h3 className="font-medium truncate">{note.id === currentNote?.id ? currentNote.note_title : note.note_title}</h3>
                                <p className="w-40 text-sm text-muted-foreground truncate">{note.id === currentNote?.id ? currentNote?.note_content : note.note_content.substring(0, 60)}</p>
                            </div>
                            <Button
                                variant="ghost"
                                size="icon"
                                onClick={async (e) => {
                                    ...
                                    toast("Note deleted", {type: "success", autoClose: 1000});
                                }}
